...
resolveRelativeNames fullPrgm@(Prgm fullPrgmObjMap _ _) (Prgm objMap (ClassGraph cg) annots) = mapMetaPrgm resolveMeta (Prgm objMap classGraph' annots)
  where
    ...
    mapType reqResolve (UnionType Nothing PosPartials partials []) = UnionType Nothing PosPartials (joinUnionType $ map (mapPartial reqResolve) $ splitUnionType partials) []
    mapType _ t = t
