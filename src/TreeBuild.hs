...
envLookup (_, _, classMap) _ input _ srcType destType | hasPartial classMap srcType destType = return input
...
envLookup env@(_, _, classMap) obj input visitedArrows srcType@(PClassName _, _, _, _) destType = do
