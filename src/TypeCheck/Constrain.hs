...
executeConstraint env con@(EqualsKnown _ _ pnt tp) = case sequenceT (descriptor env pnt, descriptorConVaenv env con) of
...
executeConstraint env (EqPoints _ _ (Meta _ _ (VarMetaDat p1 _)) (Meta _ _ (VarMetaDat p2 _))) | p1 == p2 = (True, env)
executeConstraint env1 con@(EqPoints _ _ p1 p2) = case sequenceT (descriptor env1 p1, descriptor env1 p2, descriptorConVaenv env1 con) of
...
executeConstraint env@FEnv{feClassGraph} con@(BoundedByKnown _ _ subPnt boundTp) = do
  ...
  case subScheme of
    ...
    TypeCheckResult _ (SType act req desc) -> do
      ...
      (True, env')
executeConstraint env@FEnv{feUnionAllObjs, feClassGraph} con@(BoundedByObjs _ _ pnt) = do
  ...
  case sequenceT (scheme, boundScheme, descriptorConVaenv env con) of
    ...
    TypeCheckResult _ (ub, objMapBoundUb, vaenv) -> do
      ...
      (False, env')
executeConstraint env con@(ArrowTo _ _ srcPnt destPnt) = do
    ...
    TypeCheckResult _ (srcUb, destUb) -> do
      ...
      case constrained of
        TypeCheckResult _ (srcUb', destUb') -> do
          ...
          (False, env'')
        TypeCheckResE _ -> (True, env)
executeConstraint env con@(PropEq _ _ (superPnt, propName) subPnt) = do
  ...
  case sequenceT (superScheme, subScheme) of
    ...
    (TypeCheckResult _ _) ->
      case sequenceT (superScheme, subScheme) of
        ...
        TypeCheckResE _ -> (True, env)
executeConstraint env con@(AddArg _ _ (srcPnt, newArgName) destPnt) = do
...
executeConstraint env@FEnv{feClassGraph} con@(PowersetTo _ _ srcPnt destPnt) = do
  ...
  case sequenceT (srcScheme, destScheme) of
    ...
    TypeCheckResult _ (ub1, ub2) -> do
      ...
      (False, env')
executeConstraint env@FEnv{feClassGraph} con@(UnionOf _ _ parentPnt childrenM) = do
